...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		botplayTxt = new FlxText(400, timeBarBG.y + 55, FlxG.width - 800, "YOUR AR NOOB", 32);
	...
	function openChartEditor()
	{
		...
		#if FEATURE_DISCORD_RPC
		DiscordClient.changePresence("Editor De Chart", null, null, true);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if (((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead)
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Fim De Jogo - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.getCharacter());
				#end
				isDead = true;
				return true;
			}
