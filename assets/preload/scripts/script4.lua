...
function onStartCountdown()
	...
	makeLuaText('scoreTxtLua', 'Score: 0 | Accuracy: ? (0%)', getTextWidth('scoreTxt'), getProperty('scoreTxt.x'), getProperty('scoreTxt.y'))
	addLuaText('scoreTxtLua')

	setTextSize('scoreTxtLua', getTextSize('scoreTxt'))
	setProperty('scoreTxt.visible', false)

	setTextFont("scoreTxtLua", "vcr.ttf");
	setTextSize("scoreTxtLua", 18)
	getObjectOrder(('healthBar') - 1)
...
function onUpdatePost(elapsed)
	...
	setProperty('scoreTxtLua.x', getProperty('scoreTxt.x') + 227)
	setProperty('scoreTxtLua.y', getProperty('scoreTxt.y') - 10)
	...
	if missesTxtExist == 'false' and tnhTxtExist == 'false' then
        setTextString('scoreTxtLua', (botPlay and 'Botplay Score: '..botPlayScore or 'Score: '..score)..'/'..maxScore..' ('..scorePer..'%)'..(not botPlay and ' | Accuracy: '..ratingName..' ('..accuracyLua..'%)' or ''))
	elseif missesTxtExist == 'true' and tnhTxtExist == 'true' then
        setTextString('scoreTxtLua', (botPlay and 'Botplay Score: '..botPlayScore or 'Score: '..score)..'/'..maxScore..' ('..scorePer..'%)'..(not botPlay and (misses == 0 and ' | Accuracy: '..ratingName..' ('..accuracyLua..'%)' or '')))
	...
	setProperty('scoreTxtLua.color', getColorFromHex((health > 1.6 and '31B0D1' or
		(health > 0.4 and health < 1.6 and (misses == 0 and not botPlay and 'FFFFFF' or 'FFFFFF')
		or (misses == 0 and not botPlay and 'FF0000' or 'FF0000')))))
end
